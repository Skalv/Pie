...
export default class ListenerOrchestrator {
  ...
  async startListener(listenerId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Cannot start ${config?.chainId || "unconfigured"} listener.`,
        error,
      );
  ...
  private updateListenerStatus(
    ...
    this.pubsub.publish(
      "listener:status",
      this.listenerStatuses.get(listenerId),
    );
  }
  ...
  private canRestartListener(listenerId: string): boolean {
    ...
    const recentAttempts = attempts.filter(
      (timestamp) =>
        now.getTime() - timestamp.getTime() < this.RESTART_WINDOW_MS,
    );
  ...
  async getListenerStatus(
    listenerId: string,
  ): Promise<ListenerStatus | undefined> {
  ...
  public async callListener(
    listenerId: string,
    method: keyof BlockchainListener,
    params: any,
  ): Promise<unknown> {
    const listener = this.listeners.get(listenerId);
    if (!listener) {
    ...
    return await listener.callMethod(method, params)
  }
}
